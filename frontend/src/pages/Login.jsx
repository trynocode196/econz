...
import { Sun, Moon, Loader2, ShieldCheck } from 'lucide-react';
...
export default function Login() {
  ...
  // Handle Google OAuth Redirect Response (both access_token hash and auth code query)
  useEffect(() => {
    ...
    handleOAuthCallback();
  }, [location, loginWithGoogle, navigate, showToast]);
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      let clientId = '171082207472-qotdfg7ul94pmk94gshds12kcurs0lm1.apps.googleusercontent.com';
      try {
        const cfgRes = await api.get('/auth/google/config');
        if (cfgRes.data?.clientId) clientId = cfgRes.data.clientId;
      } catch (e) {}

      // Direct full-page OAuth redirect
      const redirectUri = `${window.location.origin}/login`;
      const scope = encodeURIComponent('openid email profile https://www.googleapis.com/auth/documents https://www.googleapis.com/auth/drive https://www.googleapis.com/auth/drive.file');
      ...
      showToast('Could not initiate Google authentication', true);
      setGoogleLoading(false);
    }
  };
    ...
    <div className="login-bg">
      ...
      <div style={{
        ...
        <div className="glass-panel-login" style={{
          ...
          transition: 'transform 0.5s', borderRadius: '1.75rem', padding: '0.65rem'
        }}>
          <div style={{
            position: 'absolute',
            inset: 0,
          ...
          <EconzLogo size={52} style={{ position: 'relative', zIndex: 10 }} />
        </div>
      ...
      <div className="glass-panel-login" style={{
        ...
        <div>
          ...
          <p style={{
            ...
            Sign in with your Google Workspace account to access contracts, pricing, and orders.
          </p>
        </div>
        ...
        <div>
          ...
          >
            ...
            <span>{googleLoading ? 'Entering Orbit...' : 'Continue with Google'}</span>
          </button>
        </div>
        ...
        <div style={{
          ...
          gap: '0.4rem',
          fontSize: '0.75rem',
          color: dark ? 'rgba(148, 163, 184, 0.8)' : '#94a3b8',
          fontWeight: 600
        }}>
          <ShieldCheck size={14} color="#10b981" />
          <span>Secure Google Single Sign-On (SSO)</span>
        </div>
      </div>
    </div>
  );
