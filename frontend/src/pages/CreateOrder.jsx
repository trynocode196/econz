...
export default function CreateOrder() {
  ...
  const activeQuoteId = quoteId || id;
  const isEditMode = Boolean(activeQuoteId);
    ...
    const initWizard = async () => {
      ...
      try {
        const requests = [
          ...
          api.get('/products'),
          api.get('/templates'),
          ...(activeQuoteId ? [api.get(`/quotes/${activeQuoteId}`)] : []),
        ];
        ...
        const quoteRes = activeQuoteId ? results[3] : null;
        ...
        if (activeQuoteId && quoteRes?.data) {
  ...
  const calculateSkuMath = (sku, skuName, qtyInput, discountPct, sellingPrice, plan, currentCurrency, currentDealType, manualListPriceInput) => {
    ...
    const priceManually = matchedMaster ? !!matchedMaster.priceManually : false;

    let listPrice = 0;
    if (priceManually) {
      if (manualListPriceInput !== undefined && manualListPriceInput !== null && manualListPriceInput !== '') {
        listPrice = parseFloat(manualListPriceInput) || 0;
      } else if (sku.listPrice !== undefined && sku.listPrice !== null) {
        listPrice = parseFloat(sku.listPrice) || 0;
      }
  ...
  const handleSkuFieldChange = (index, field, value) => {
    setOrderSkus(prev => {
      ...
      const listPriceVal = field === 'listPrice' ? value : target.listPrice;

      updated[index] = calculateSkuMath(target, nameVal, qtyVal, discVal, sellVal, planVal, currency, dealType, listPriceVal);
      return updated;
    });
  };
  ...
  useEffect(() => {
    setOrderSkus(prev => prev.map(s =>
      calculateSkuMath(s, s.name, s.qty, s.googleDiscPct, s.sellPrice, s.subPlan, currency, dealType, s.listPrice)
    ));
  ...
  const persistQuote = async (status, title, successMessage, actionType = 'send') => {
  ...
  const handleSaveDraft = async () => {
    ...
    await persistQuote(status, editingTitle, isEditMode ? 'Order updated successfully' : 'Order saved as Draft!', 'save');
  };

  const handleSendDocuSign = async () => {
    await persistQuote('Sent for Signature', 'Signed Order Form', 'Agreement sent via BoldSign! Email OTP delivered to client.', 'send');
  };

  const handleRequestApproval = async () => {
    await persistQuote('Pending Approval', 'Exception Order', 'Exception approval requested. Manager notified.', 'approval');
  };
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '960px', margin: '0 auto', width: '100%', paddingBottom: '3rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }} className="grid-2">
                ...
                <div>
                  ...
                  <input type="text" list="designations-list" value={pocDesignation} onChange={(e) => setPocDesignation(e.target.value)} className="input-orbit" placeholder="Start typing..." required />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }} className="grid-2">
                  <div>
                    <label className="field-label" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span>List Price {sku.priceManually && <span style={{ color: '#0ea5e9', fontSize: '0.75rem', fontWeight: 600 }}>(Manual)</span>}</span>
                    </label>
                    {sku.priceManually ? (
                      <div style={{ position: 'relative' }}>
                        <span style={{ position: 'absolute', left: '0.75rem', top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)', fontWeight: 700, fontSize: '0.8125rem' }}>{curSym}</span>
                        <input
                          type="number"
                          value={sku.listPrice !== undefined && sku.listPrice !== null ? sku.listPrice : ''}
                          onChange={(e) => handleSkuFieldChange(idx, 'listPrice', e.target.value)}
                          className="input-orbit"
                          placeholder="0.00"
                          min="0"
                          step="0.01"
                          style={{ paddingLeft: '2rem', fontWeight: 700 }}
                        />
                      </div>
                    ) : (
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              ...
              {approvalRequired ? (
                ...
                >
                  {isSubmitting && submittingAction === 'approval' ? (
                    <Loader2 size={15} className="animate-spin text-white" />
                  ) : (
                    <Send size={15} />
                  )}
                  <span>{isSubmitting && submittingAction === 'approval' ? 'Requesting...' : 'Request Manager Approval'}</span>
                </button>
              ) : (isEditMode && (editingStatus === 'Customer Signed' || editingStatus === 'Signed' || editingStatus === 'Completed')) ? (
                <span
                  style={{
                    display: 'inline-flex',
                    alignItems: 'center',
                ...
                >
                  ...
                  <span>
                    {isSubmitting && submittingAction === 'send'
                      ...
                      : (isEditMode && editingStatus === 'Sent for Signature' ? 'Resend Via BoldSign' : 'Send Via BoldSign')}
                  </span>
                </button>
              )}
            </div>
      ...
      <SectionGuideModal
        ...
        guide={SECTION_GUIDES.createOrder}
      />
    </div>
  );
}
