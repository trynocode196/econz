...
export default function ProductCatalog() {
  ...
  const getProductIcon = (key) => {
    switch(key) {
      case 'GWS': return <Mail size={32} className="text-red-500" />;
      case 'GCP': return <Cloud size={32} className="text-blue-500" />;
      default: return <Layout size={32} className="text-amber-500" />;
    }
  };

  const getProductBg = (key) => {
    switch(key) {
      case 'GWS': return 'rgba(239, 68, 68, 0.05)';
      case 'GCP': return 'rgba(59, 130, 246, 0.05)';
